# Investigation Notes - Lab 75 IFEO Debugger Abuse

## Investigation Focus

This investigation examines a controlled Windows Image File Execution Options (IFEO) configuration for `notepad.exe`.

The main focus was to determine:

- Whether an IFEO key existed for the target.
- Whether a `Debugger` value was already present.
- What debugger executable was configured.
- Whether the debugger executable was legitimate.
- What process activity was generated during the lab.
- Whether Sysmon provided useful process telemetry.
- Whether Wazuh provided supporting telemetry.
- Whether the observed process activity could be confidently attributed to IFEO.
- Whether the test configuration was successfully removed.

The investigation was performed in a controlled Windows 11 lab environment without using a malicious payload.
...
## 18. Investigation Takeaways

### What was confirmed?

The controlled IFEO `Debugger` configuration was successfully created and verified.

### What was not confirmed?

The available process telemetry did not conclusively prove that an observed `cmd.exe` process was launched through the IFEO configuration.

### Why?

The environment contained multiple legitimate `cmd.exe` processes, and the observed Notepad process used a WindowsApps/MSIX path instead of the targeted System32 executable.

### Analyst lesson

Do not classify IFEO activity based only on the presence of:
...
Correlate the Registry artifact with process creation, executable paths, parent-child relationships, command lines, timestamps, and available endpoint telemetry.
