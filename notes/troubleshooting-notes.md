# Troubleshooting Notes - Lab 75 IFEO Debugger Abuse
...
|---|---|---|
| `reg.exe` invalid key name | PowerShell `HKLM:\` syntax used with `reg.exe` | Use `HKLM\` |
| Debugger remained after PowerShell cleanup | Value was still present | Remove with `reg.exe delete` |
| Multiple `cmd.exe` processes | Legitimate applications also create `cmd.exe` | Correlate parent/child process data |
| Notepad path mismatch | WindowsApps Notepad was observed | Validate full executable path |
| `wazuh-archives-*` unavailable | Index not present | Use available Wazuh telemetry and document gap |
| IFEO execution not proven | Insufficient process correlation | Classify attribution as inconclusive |
| Registry modification event unavailable | Required telemetry not established | Document evidence limitation |

---

## Troubleshooting Principle

> **Do not treat a command result as proof until the expected system state has been independently verified.**

This was particularly important during the cleanup phase, where the PowerShell command returned without an obvious error but the `Debugger` value remained present.
